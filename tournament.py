...
from utils import convert_currency, extract_hierarchical_info, compute_risk
from models import *
from download import download_tickers_info, get_exchange_rates
...
if __name__ == "__main__":
    ...
    cli.add_argument(
        ...
        type=float,
        default=100000.0,
        help="Bots start with this available capital at " "the beginning of the tournament. ",
    )
    ...
    cli.add_argument(
        ...
        type=str,
        default=onemonthago,
        help="Approximate initial date of the bot-tournament. Format: " "YY-MM-DD`.",
    )
    cli.add_argument(
        "--end", type=str, default=today, help="Approximate final date of the bot-tournament. Format: " "YY-MM-DD`."
    )
    args = cli.parse_args()
    ...
    data = download_tickers_info(args.symbols, start=start_download, end=args.end)
    ...
    if args.currency == data["default_currency"]:
        ...
        xrate = get_exchange_rates(
            [args.currency], data["default_currency"], data["dates"], start=args.start, end=args.end
        )[args.currency][-num_days]
    args.capital *= xrate
    ...
    for j in range(num_days, 0, -1):
        ...
        bot_info = {
            tickers[i]: {"price": price[i, -j - 1], "rate": rates[i], "growth": growth[i], "scores": scores[i]}
        ...
        for i, (name, bot) in enumerate(tournament.items()):
            ...
            print(
                str_format.format(
                    ...
                    " ".join(map(str, list(bot.portfolio.keys())[:10]))
                    + ("..." if len(bot.portfolio.keys()) > 10 else ""),
